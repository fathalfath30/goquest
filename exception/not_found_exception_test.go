...
func (ts *ExceptionTestSuite) Test_NotFoundException() {
	ts.Run("it can create new HttpRequestException and return default value if message are empty", func() {
		actual := exception.NewNotFoundException("lorem ipsum")

		ts.Require().NotNil(actual)
		ts.Require().IsType(&exception.NotFoundException{}, actual)
